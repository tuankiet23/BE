with time_filtered_job as (
   ...
   , all_job as (
    ...
    from jobs j
             join status_job js on j.status_job_id = js.id
    where js.status_name in ('RECRUITING', 'STOP', 'COLSED')
   ...
   , not_done_job_temp as (
    ...
    from time_filtered_job j
             ...
             left join profile_status ps on ps.id = jr.profile_status_id and ps.name = 'SUCCESS'
   ...
   , waiting_for_interview as (
    select count( jr.id) waiting_for_interview
    from time_filtered_job j
             inner join job_register jr on jr.job_id = j.id
             inner join profile_status ps on ps.id = jr.profile_status_id and ps.name = 'INTERVIEW'
)
   , interviewing as (
    select count(jr.id) interviewing
    from time_filtered_job j
             inner join job_register jr on jr.job_id = j.id
             inner join profile_status ps on ps.id = jr.profile_status_id and ps.name = 'ACCEPTED_INTERVIEW'
)
   , total_apply as (
